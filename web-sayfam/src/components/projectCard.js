...
const ProjectCard = ({ project }) => {
  ...
  return (
    <div className="flex flex-wrap bg-white dark:bg-my-black rounded-xl mb-8 gap-4 shadow-lg">
      <img
        className="size-[22.5rem]  object-cover rounded-l-xl"
        src={require("../images/" + `${photo}`)}
      />
      <div className="flex text-left">
        <div className="px-8 flex justify-center grow flex-col gap-6">
          <h4 className="text-my-purple font-bold text-3xl font-inter mb-0">
            {project.projname}
          </h4>
          <p className="mb-0 max-w-2xl">{project.explaination}</p>
          ...
          <div>
            <a
              href={project.href}
              className="text-black dark:text-my-green font-medium underline-offset-4 mr-4"
            >
              View Site
            </a>
            <a
